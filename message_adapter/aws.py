"""AWS service endpoint configuration for cumulus-message-adapter."""
...
def localhost_s3_url():
    """Return configured LOCALSTACK_HOST url or default for localstack s3."""
...
def s3():
    """Determine and return the endpoint for the S3 service."""
    ...
    if ("CUMULUS_ENV" in os.environ) and (os.environ["CUMULUS_ENV"] == "testing"):
        return resource(
            ...
            aws_secret_access_key="my-secret",  # noqa: S106
...
def _get_sfn_execution_arn_by_name(state_machine_arn, execution_name):
    """Return the ARN for a state machine execution given its name.

    * @param {string} state_machine_arn The ARN of the state machine containing the
    *                                   execution
...
def _get_task_name_from_execution_history(execution_history, arn):
    """Return the most recent task name for the given ARN from execution history.
    ...
    IMPORTANT! If no ARN is supplied, this message assumes that the most recently
    started execution is the desired execution. This WILL BREAK parallel executions,
    so always supply this if possible.

    * @param {dict} executionHistory The execution history returned by
    *                                getExecutionHistory, assumed to be sorted so most
    *                                recent executions come last
