"""Cumulus message loading, templating, and remote storage utilities."""
...
def load_config(event):
    """Return the config object for the task from a Cumulus message.

    * @param {*} event An event in the Cumulus message format with remote parts resolved
    * @param {*} context The context object passed to AWS Lambda or containing an
    *                    activityArn
...
def load_remote_event(event):
    """Load a remote event from S3 if a 'replace' key is present, otherwise return
    as-is.
    ...
    * @returns {*} A Cumulus message with the remote message resolved
    ...
    if "replace" in event:
        ...
        if data is not None:
            ...
            if exception_bool and (
                not event["exception"] or event["exception"] == "None"
            ):
...
def resolve_path_str(event, json_path_string):
    """Interpret a JSONPath template string against a Cumulus message and return the
    result.
    ...
    * @returns {*} The resolved object
    ...
    if re.search(array_regex, json_path_string):
        parsed_json_path = (
            json_path_string.lstrip("{").rstrip("}").lstrip("[").rstrip("]")
        )
...
def resolve_input(event, config):
    """Return the input object to send to the task, as defined under
    config.cumulus_message.
...
def resolve_config_templates(event, config):
    """Resolve all JSONPath-templated values in a config object using the provided
    event.
...
def store_remote_response(incoming_event, default_max_size, config_keys):
    """Store part of a response message in S3 if it exceeds the StepFunctions size
    limit.

    * @param {*} incoming_event    - The response message
    * @param {*} default_max_size  - The maximum size (in bytes) a response message
    *                                portion can be before storing it in s3
    * @param {*} config_keys       - A list of valid CMA configuration keys
    * @returns {*} A response message, possibly referencing an S3 object for its
    *              contents
    """
    ...
    replace_config_values = _parse_remote_config_from_event(
    ...
    if len(replacement_data) != 1:
        raise ValueError(
            f"JSON path invalid: {replace_config_values['parsed_json_path']}"
        )
...
def _resolve_config_object(event, config):
    """Recursively resolve JSONPath-templated values in a config object.
...
def _parse_remote_config_from_event(replace_config, default_max_size):
    """Parse and return remote storage configuration from the event's ReplaceConfig."""
