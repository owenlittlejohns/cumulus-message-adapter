"""MessageAdapter class for transforming Cumulus workflow messages."""
...
class MessageAdapter:
    ...
    def __init__(self, schemas=None):
        """Initialize MessageAdapter with optional schema configuration."""
    ...
    def load_and_update_remote_event(self, incoming_event, context):
        """Load and return the full Cumulus message, fetching remote S3 data if present.

        If the message has part of its data stored remotely in S3, fetches that data,
        otherwise it returns the full message, both cases updated with task metadata.
        If event uses parameterized configuration, converts message into a Cumulus
        message and ensures that incoming parameter keys are not overridden.
        ...
        * @returns {*} the full event data
        ...
        if context and "meta" in event:
            task_meta = {}
            task_meta["name"] = context.get(
                "function_name", context.get("functionName")
            )
            task_meta["version"] = context.get(
                "function_version", context.get("functionVersion")
            )
            task_meta["arn"] = context.get(
                "invoked_function_arn",
                context.get("invokedFunctionArn", context.get("activityArn")),
            )
            if "workflow_tasks" not in event["meta"]:
    ...
    def __get_jsonschema(self, schema_type):
        ...
        rel_filepath = (
            schemas.get(schema_type) if has_schema else f"schemas/{schema_type}.json"
        )
    ...
    def load_nested_event(self, event):
        """Interpret an incoming event as a Cumulus workflow message.
        ...
        * @returns {*} message that is ready to pass to an inner task
        ...
        # add cumulus_config property, only selective attributes from event.cumulus_meta
        if "cumulus_meta" in event:
            ...
            if all(attribute in event["cumulus_meta"] for attribute in attributes):
                for attribute in attributes:
                    response["cumulus_config"][attribute] = event["cumulus_meta"][
                        attribute
                    ]
    ...
    def __assign_outputs(handler_response, event, message_config):
        """Apply a task's return value to an output message as defined in
        cumulus_message.
    ...
    def create_next_event(self, handler_response, event, message_config):
        """Create and return the output message returned by a task.
        ...
        * @returns {*} the output message to be returned
        ...
        if "replace" in result:
            del result["replace"]
        return store_remote_response(
            result, self.REMOTE_DEFAULT_MAX_SIZE, self.CMA_CONFIG_KEYS
        )
